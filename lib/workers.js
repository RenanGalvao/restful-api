      ...
      checks.forEach(check => {
        ...
        _data.read('checks', check, (err, originalCheckData) => {
          if(!err && originalCheckData){
            ...
            console.log('Error: Could not read one of the check\'s data.');
          }
        });
      });
    }else{
  ...
  _data.update('checks', newCheckData.id, newCheckData, err => {
    if(!err){
      ...
      if(alertWarranted){
        ...
        console.log('Check outcome has not changed, no alert neeeded.');
      }
    }else{
      console.log('Error: Could not save the updates to one of the checks.');
    }
  });
};
...
workers.alertUserToStatusChange = (newCheckData) => {
  ...
  helpers.sendTwilioSms(newCheckData.userPhone, msg, err => {
    if(!err){
      console.log('Error: Could not send sms alert to user who had a state change in their checks.');
    }else{
          ...
          if(!err){
            ...
            _logs.truncate(logId, err => {
              if(!err){
                ...
                console.log('Error: could not truncate the file.');
              }
            });
          }else{
            console.log('Error: Could not compress one of the log files', err);
          }
        });
      });
    }else{
      console.log('Error: Could not find any logs to routate');
    }
  });
};
