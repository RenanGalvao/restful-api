...
export default {
  ...
  async deleted(request){
    debug(formatWithOptions({colors: true}, '[ACCOUNT][DELETED] Request: %O', request));
    ...
    try{
      ...
      const payload = await getTemplate('account', 'deleted', data);
      debug(formatWithOptions({colors: true}, '[ACCOUNT][DELETED] Response: %O', payload));
      return {status: 200, payload, contentType: 'text/html'};
    }catch(err){
      return errorHandler(err);
    }
