...
export default {
  ...
  async edit(request){
    debug(formatWithOptions({colors: true}, '[CHECKS][EDIT] Request: %O', request));
    ...
    try{
      ...
      const payload = await getTemplate('checks', 'edit', data);
      debug(formatWithOptions({colors: true}, '[CHECKS][EDIT] Response: %O', payload));
      return {status: 200, payload, contentType: 'text/html'};
    }catch(err){
      return errorHandler(err);
    }
