...
export default {
  ...
  async deleted(request){
    debug(formatWithOptions({colors: true}, '[SESSION][DELETED] Request: %O', request));
    ...
    try{
      ...
      const payload = await getTemplate('session', 'deleted', data);
      debug(formatWithOptions({colors: true}, '[SESSION][DELETED] Response: %O', payload));
      return {status: 200, payload, contentType: 'text/html'};
    }catch(err){
      return errorHandler(err);
    }
