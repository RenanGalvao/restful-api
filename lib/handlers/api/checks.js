...
export default {
  ...
  async get(request) {
    ...
    if(!id){
      return {status: 400, payload: {err: 'Validation', message: `Missing /:id`}, contentType: 'application/json'};
    }
    ...
    if(!userData.checks.includes(id)){
      return {status: 400, payload: {err: 'Bad Request', message: 'This check doesn\'t belongs to you.'}, contentType: 'application/json'};
    }

    debug(formatWithOptions({colors: true}, '[CHECKS][GET] Response: %O', check));
    return {status: 200, payload: check, contentType: 'application/json'};
  },
  ...
  async put(request) {
    ...
    return {status: 200, payload: checkData, contentType: 'application/json'};
  },
  ...
  async delete(request) {
    ...
    const userData = await _data.read('users', request.token.phone);
    if(!userData.checks.includes(id)){
      return {status: 400, payload: {err: 'Bad Request', message: 'This check doesn\'t belongs to you or doesn\'t exists.'}, contentType: 'application/json'};
    }
