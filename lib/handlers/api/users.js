...
import { validate, invalidFields, hash, deleteCookieHeader } from '../../helpers.js';
...
export default {
  ...
  async post(request) {
    ...
    try{
      ...
      return {status: 400, payload: {err: 'Bad Request', message: 'A user with that phone already exists.'}, contentType: 'application/json'};
    }catch{
      // Hash password
  ...
  async delete(request) {
    ...
    if(!phone){
      return {status: 400, payload: {err: 'Validation', message: `Missing /:phone`}, contentType: 'application/json'};
    }

    // Look up user data and compare with the sent phone
    const userData = await _data.read('users', request.token.phone);
    if(userData.phone != phone){
      return {status: 400, payload: {err: 'Bad Request', message: 'You cannot delete others\' account.'}, contentType: 'application/json'};
    }
