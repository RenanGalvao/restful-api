...
export default {
  ...
  async post(request) {
    ...
    const [accessToken, refreshToken] = await createTokens(phone, tokenIdLength, accessTokenMaxAge, refreshTokenMaxAge);
      
    debug(formatWithOptions({colors: true}, '[LOGIN] Access Token: %O\nRefresh Token: %O', accessToken, refreshToken));
    return {status: 200, payload: {auth: true}, contentType: 'application/json', headers: generateCookieHeader(accessToken, refreshToken)};
