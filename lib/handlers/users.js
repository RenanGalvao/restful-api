...
export default {
  ...
  async put(request) {
    ...
    const password = validate(request.payload.password, 'string');
    debug(formatWithOptions({colors: true}, '[USERS][POST] Validation: %O', {phone, firstName, lastName, optionalPhone, password}));
    ...
    if(firstName || lastName || optionalPhone || password){
      ...
      // Save
      if(optionalPhone != request.token.phone){
  ...
  async delete(request) {
    ...
    // Inside all API handlers, slug[0] is 'api', the next one is our phone
    ...
    if(phone){
      ...
      const userData = await _data.read('users', request.token.phone);
      if(userData.phone != request.token.phone){
