...
helpers.generateCookieHeader = (access_token, refresh_token) => {
  ...
  return {
    'Set-Cookie': [
      `access_token=${access_token.id}; Expires=${access_token.expires}; HttpOnly; Path=/; SameSite=Lax; ${process.env.NODE_ENV == 'production' ? 'Secure' : ''};`,
      `refresh_token=${refresh_token.id}; Expires=${refresh_token.expires}; HttpOnly; Path=/; SameSite=Lax; ${process.env.NODE_ENV == 'production' ? 'Secure' : ''};`  
    ]
  }
};
...
*/
helpers.createTokens = async (phone, tokenIdLength, accessTokenExpires, refreshTokenExpires) => {
  const access_token = {
    id: createRandomString(tokenIdLength),
    phone,
    expires: Date.now() + accessTokenExpires,
  };
  const refresh_token = {
    id: createRandomString(tokenIdLength),
    phone,
