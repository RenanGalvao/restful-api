...
*/
helpers.getTemplate = (dir, templateName, custom_err = {}) => {
  ...
  return new Promise((resolve, reject) => {
    fs.readFile(path.join(templateBaseDir, dir, `${templateName}.html`), 'utf-8', (err, data) => {
      if(!err && data){
        debug(formatWithOptions({colors: true}, '[HELPERS][GET_TEMPLATE] Response: %O', data));
        resolve(data);
      }else{
