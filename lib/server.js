...
server.unifiedServer = (req, res) => {
  ...
  req.on('end', async () => {
    ...
    const choosedHandler = (path == 'api' && slugs.length > 0 ? server.router[path][slugs[0]] : path != 'api' && typeof server.router[path] !== 'undefined' ? server.router[path] : handlers.notFound);
