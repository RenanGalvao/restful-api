...
server.unifiedServer = (req, res) => {
  ...
  req.on('end', async () => {
    ...
    const choosedHandler = (path == 'api' && slugs.length > 0 && typeof server.router[path][slugs[0]] != 'undefined' ? server.router[path][slugs[0]] : path != 'api' && typeof server.router[path] != 'undefined' ? server.router[path] : handlers.notFound);
    ...
    debug(formatWithOptions({colors: true}, '%O', data));

    // Handle the response
    try{
      const response = await choosedHandler(data);

      // Convert the payload from JSON to string
